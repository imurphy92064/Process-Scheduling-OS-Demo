...
int main(int argc, char *argv[]) {
    ...
    cout << "CS 433 Programming assignment 3\n" <<
            ...
            "Description: Priority Round Robin (RR) CPU scheduling algorithmn implementation using a map and queue.\n" <<
            "=================================" << endl;

    /* Check Arguments */
    if (argc < 2) {
        cerr << "Usage: ./rr_priority <input_file> [<time_quantum>]" << endl;
        exit(1);
    }
    ...
    map<int, queue<Task *>> queued_tasks;
    ...
    while (getline(file, line)) {
        ...
        queued_tasks[priority].push(task);
    ...
    /* Begin scheduling */
    int current_time = 0;
    map<int, queue<Task *>>::reverse_iterator itr = queued_tasks.rbegin();
    while (itr != queued_tasks.rend()) {
        while (!queued_tasks[itr->first].empty()) {
            Task *task = queued_tasks[itr->first].front();

            int current_burst_time = (task->remaining_cpu_burst() - time_quantum < 0 || queued_tasks[itr->first].size() == 1) ? task->remaining_cpu_burst() : time_quantum;
            ...
            if (task->remaining_cpu_burst() <= 0) {
                ...
                queued_tasks[itr->first].push(task);
            }
        }
        itr++;
    }
